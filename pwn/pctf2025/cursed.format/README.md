...
- We have to leak stack and libc addresses and write inside the stack with a crafted rop chain to execute ``system("/bin/sh")``. This can be tricky because of the 0x20 bytes problem, we need to write `pop rdi; ret + bin_sh address + ret; + system()`, 4 full addresses (32 bytes).
...
### Solvers
- You can download the solver script [here](assets/local.py)
